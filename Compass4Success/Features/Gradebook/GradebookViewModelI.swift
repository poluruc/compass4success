...
class GradebookViewModelI: ObservableObject {
    ...
    func updateGrade(for student: Student, assignment: Assignment, newGrade: Double?) {
        ...
        if let submissionIndex = assignment.submissions.firstIndex(where: { $0.studentId == student.id }) {
            // We can only modify mutable properties on the submission
    ...
    func studentSubmissionStatus(for student: Student, assignment: Assignment) -> GradebookISubmissionStatus {
        if let submission = assignment.submissions.first(where: { $0.studentId == student.id }) {
    ...
    // Convert CoreSubmissionStatus to GradebookISubmissionStatus
    private func statusToGradebookStatus(_ status: CoreSubmissionStatus) -> GradebookISubmissionStatus {
        switch status {
...
enum GradebookISubmissionStatus: String {
